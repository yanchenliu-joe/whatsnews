"""DeepSeek provider — placeholder (Phase 22+).

Implement by:
1. pip install openai  (DeepSeek is OpenAI-compatible)
2. Set DEEPSEEK_API_KEY
3. Use openai.OpenAI(base_url="https://api.deepseek.com", api_key=...).
4. Register in ai/registry.py: _PROVIDERS["deepseek"] = DeepSeekProvider()
"""
...
class DeepSeekProvider(AIProvider):
    ...
    def chat_completion(self, *, operation, model, messages, max_tokens, temperature, response_format=None) -> ChatResult:
        raise NotImplementedError("DeepSeekProvider.chat_completion not implemented")

    async def stream_chat(self, *, operation, model, messages, max_tokens, temperature) -> AsyncIterator[str]:
        raise NotImplementedError("DeepSeekProvider.stream_chat not implemented")
        yield

    def text_to_speech(self, *, operation, model, voice, text, count_as_generation=True) -> bytes:
        raise NotImplementedError("DeepSeekProvider.text_to_speech not implemented")
